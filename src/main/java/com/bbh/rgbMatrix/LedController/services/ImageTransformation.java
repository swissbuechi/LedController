...
public class ImageTransformation {
    ...
    public BufferedImage getScaledImage(BufferedImage image, Dimension newDim) {
        ...
        double scaleX = (double) newDim.getWidth() / image.getWidth();
        double scaleY = (double) newDim.getHeight() / image.getHeight();
     ...
     */
    public BufferedImage getSubimage(BufferedImage image, List<Integer> scale) {
        try {
            int startPosX = scale.get(0);
            int startPosY = scale.get(1);
            int width = scale.get(2);
            int height = scale.get(3);
            return image.getSubimage(startPosX, startPosY, width, height);
        } catch (RasterFormatException Rfe) {
            System.err.println("Cropping not possible, selected part is outside of the image");
            System.exit(1);
        }
        return null;
    }
